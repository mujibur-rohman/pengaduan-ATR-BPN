...
function isi_array() {
    ...
    for (var i = 0; i < json_object.length; i++) {
        ...
        products.push({
            NO_REFERENSI: json_object[i].NO_REFERENSI, NO_KPPBC: json_object[i].KD_KANTOR, TGL_PPE: json_object[i].TGL_DAFTAR, NO_PPE: json_object[i].NO_PPE,
    ...
    load_datagrid(productsStore, ordersStore);
        ...
        $("#grid-container").dxDataGrid({
            ...
            columns: [
                ...
                { caption: "TGL PPE", dataField: "TGL_PPE", dataType: "date", displayFormat: "dd-MM-yyyy"},
                { caption: "NO PPE", dataField: "NO_PPE", dataType: "string" },
                    ...
                    $("<div>").appendTo(container).dxDataGrid({
                        ...
                        onRowRemoving: function (e) {
                            if (ViewOnly) {
                                ...
                                var method = "POST"; var pebJSon = routeApi + "DataDHE/LoadUpdateInvoice_PPE";
                                var ID = e.data.ID;
                                var IDREF = options.key;
                                var TOT = 0;
                                var NO_DOKUMEN = e.data.NO_PPE;
        ...
        onClick: function (e) {
            ...
            if ($("#txtPerPEB2").dxDateBox("instance").option("value") === null || $("#txtSampaiPEB2").dxDateBox("instance").option("value") === null) {
                ...
                if ($("#txtPerPEB2").dxDateBox("instance").option("value") > $("#txtSampaiPEB2").dxDateBox("instance").option("value")) {
                    ...
                    var method = "GET";
                    var urlPEB = routeApi + "DataPEB/LoadInvoicePPE";
                    var urlPEB2 = routeApi + "DataPEB/LoadInvoicePPE_GROUP";
...
$("#uploadEkspor").show().dxFileUploader({
    ...
    uploadUrl: routeApp + "Pelapor/Invoice_PPE",
    ...
    onUploaded: function (d) {
        ...
        reader.onloadend = function (dt) {
            ...
            var arrayTest = ["ID", "NO REFERENSI", "NPWP", "KD KANTOR", "NO DAFTAR", "JNS TRX", "TGL INVOICE", "NO PPE", "NO INVOICE", "VALUTA INVOICE", "NILAI FINAL", "ID SEQ","ID STATUS INVOICE"]
            ...
            $.ajax({
                url: routeApi + "Netting/UploadInvoice_PPE",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
            ...
            success: function (e) {
               ...
               for (var i = 0; i < json_object2.length; i++) {
                //   alert("group" + json_object2[i].NO_REFERENSI + "->" + json_object2[i].KD_KANTOR + "->" + json_object2[i].TGL_DAFTAR + "->" + json_object2[i].THN + "->" + json_object2[i].NO_PPE + "->" + json_object2[i].NILAI_FOB + "->" + json_object2[i].TOTAL_INVOICE)
                   products.push({
                       NO_REFERENSI: json_object2[i].NO_REFERENSI, NO_KPPBC: json_object2[i].KD_KANTOR, TGL_PPE: json_object2[i].TGL_DAFTAR, NO_PPE: json_object2[i].NO_DAFTAR, NILAI_FOB: 0,
                ...
                for (var x = 0; x < json_object3.length; x++) {
                  //  alert("detail " + json_object3[x].NO_REFERENSI + "->" + json_object3[x].TGL_DAFTAR + "->" + json_object3[x].TGL_DOK + "->" + json_object3[x].KD_VAL + "->" + json_object3[x].FOB_CIF )
                    ordersStore.push({
                        ...
                        TOT: json_object3[x].TOT,ID: json_object3[x].ID_PPE_PPI_DOK, NO_REFERENSI: json_object3[x].NO_REFERENSI,TGL_INVOICE: json_object3[x].TGL_DOK, NO_INVOICE: json_object3[x].NO_DOKUMEN, NO_PPE: json_object3[x].NO_DAFTAR, VALUTA_INVOICE: json_object3[x].KD_VAL, NILAI_FINAL: json_object3[x].FOB_CIF
