$(function () {
    ...
    $("#gridIncoming").dxDataGrid({
        ...
        columns: [
            {
                ...
                caption: "Flag PPE",
                alignment: "left",
                dataField: "flag_with_peb",
                lookup: {
                    dataSource: [
                        { id: true, text: "Dengan PPE" },
                        { id: false, text: "Tanpa PPE" }
                    ],
                    displayExpr: 'text',
                    valueExpr: 'id'
                }
        ...
        onCellPrepared: function (e) {
            if (e.rowType === "data" & e.column.command === 'select' && e.data.flag_is_migrate == 1) {
        ...
        onSelectionChanged: function (e) {
            console.log(e);
            if (e.component.skipNextSelectionChanged) {
                e.component.skipNextSelectionChanged = false;
                return;
            }
            ...
            $.each(selectedRows, function (i, v) {
                if (v.flag_is_migrate == 1)
                    rowsToDeselect.push(v);
            })

            if (e.currentSelectedRowKeys.length == rowsToDeselect.length) {
                e.component.skipNextSelectionChanged = true;
                e.component.deselectAll();
            }
            else if (rowsToDeselect.length > 0)
            {
                e.component.skipNextSelectionChanged = true;
                e.component.deselectRows(rowsToDeselect);
            }
        },
    });
        ...
        onClick: function (e) {
            if ($("#dateStart_inc").dxDateBox("instance").option("value") === null || $("#dateEnd_inc").dxDateBox("instance").option("value") === null) {
                ...
                if ($("#dateStart_inc").dxDateBox("instance").option("value") > $("#dateEnd_inc").dxDateBox("instance").option("value")) {
                    ...
                    var method = "GET";
                    var urls = routeApi + "DataInOut/IncomingAll";
