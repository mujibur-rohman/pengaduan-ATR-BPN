$(function () {
    let upData = getLocalStorage("dokupenDataNetting") === "" ? [] : JSON.parse(getLocalStorage("dokupenDataNetting"));
    ...
    $("#gridUploadExportir").hide().dxDataGrid({
        ...
        onRowInserted: function (e) {
            $("#btnClearTrx").attr("disabled", false);
            $("#btnPostNetting").attr("disabled", false);
            setLocalStorage("dokupenDataNetting", JSON.stringify(e.component.getDataSource()._items));
        },
    ...
    $("#btnClearTrx").click(function () {
        ...
        confRes.done(function (dialogResult) {
            if (dialogResult) {
                if (viewOnly) {
                    toast_notify("Anda tidak memiliki wewenang untuk membatalkan transaksi netting", "warning", 5000);
                } else {
                    cancelNetting();
                }
            }
    ...
    $("#btnPostNetting").click(function () {
        ...
        confRes.done(function (dialogResult) {
            if (dialogResult) {
                if (viewOnly) {
                    toast_notify("Anda tidak memiliki wewenang untuk menyimpan transaksi netting", "warning", 5000);
                } else {
    ...
    function setKuota(kuotaNetting, kuotaNettingUSD) {
        ...
        if (getLocalStorage("nettingStatus") === "incoming") {
            ...
            totalUSD = $("#gridNettingIncoming").dxDataGrid("instance").getTotalSummaryValue("total_usd_inc");
        ...
        if (getLocalStorage("nettingStatus") === "outgoing") {
            ...
            totalUSD = $("#gridNettingOutgoing").dxDataGrid("instance").getTotalSummaryValue("total_usd_out");
            if (totalUSD || getLocalStorage("setOff")) {
        ...
        if (getLocalStorage("setOff") === "minus" && def && usd) {
            ...
            if (getLocalStorage("nettingStatus") === "incoming") $("#nettingStatus").text("Netting Terima");
            else if (getLocalStorage("nettingStatus") === "outgoing") $("#nettingStatus").text("Netting Bayar");
            else $("#nettingStatus").text("Netting OK");
    ...
    function clearMyStorage() {
        ...
        removeLocalStorage("nettingKuota");
        removeLocalStorage("nettingKuotaUSD");
        removeLocalStorage("dokupenDataNetting");
    }
