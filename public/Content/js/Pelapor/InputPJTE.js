...
$(function () {
    ...
    var gridDataSource = new DevExpress.data.CustomStore({
        ...
        remove: function (key, dd) {
            ...
            $.ajax({
                url: URL + 'DeletePJT?trx=10&id_pel_input_pjt=' + key.id_pel_input_pjt,
                method: 'GET',
        ...
        load: function (load) {
            ...
            return $.ajax({
                url: URL + "LoadPJT?trx=10",//?ParamPEB=" + GetSelectedPEB(),
                type: "GET",
    ...
    let upData = getLocalStorage("dokupenDataPJTE") === "" ? [] : JSON.parse(getLocalStorage("dokupenDataPJTE"));
    ...
    var mapCaptionToDataField = {
        ...
        "Nilai Barang": "nilai_disbursement_pjt",
    ...
    $("#uploadEkspor").show().dxFileUploader({
        ...
        uploadUrl: routeApp + "Pelapor/InputPJTE",
        ...
        onUploaded: function (d) {
            ...
            if (viewOnly) {
                ...
                reader.onloadend = function (e) {
                    ...
                    var CountLoop = 0;
                    var arrayTest = ["NO PPE", "NPWP PJT", "NAMA PEMILIK BARANG", "NILAI BARANG", "NOMOR INVOICE", "TANGGAL INVOICE"];
                            ...
                            if (GroupSum.length !== selPEB.length) {
                                ...
                                for (i = 0; i < GroupSum.length; i++) {
                                    for (y = 0; y < selPEB.length; y++) {
                                        if (GroupSum[i]["no_peb_pib"] != selPEB[y]["NO_PEB"]) {
                                    ...
                                    if (GroupSum[i]["nilai_disbursement_pjt"] > selPEB[matchPEB]["total_disbursment_pdkm"]) {
    ...
    $("#gridUploadPDKM").dxDataGrid({
        ...
        onRowInserted: function (e) {
            $("#rowbtnSaveClear").css("display", "block");
            setLocalStorage("dokupenDataPJTE", JSON.stringify(e.component.getDataSource()._items));
        },
    ...
    var dataGrid = $("#gridSelectedPEB").dxDataGrid({
            ...
            if (viewOnly) {
                ...
                if (tDisbursment < 1) {
                    ...
                    if (tDisbursment > summaryVal) {
                        ...
                        return $.ajax({
                            url: routeApi + "DataPDKM/updateDis?tDisbursment=" + tDisbursment + "&NoPEB=" + e.oldData.ID_PEL_PEB + "&total_split=" + tsplit, //+ NPWP,
                            type: "POST",
                            dataType: "json",
                            success: function (data) {
        ...
        onRowRemoving: function (e) {
            if (viewOnly) {
                ...
                return $.ajax({
                    url: URL + "DeleteSelectedPEBPIB?trx=10&id_pel_peb_pib=" + id_pel_peb, //+ NPWP,
                    type: "POST",
                    dataType: "json",
                    success: function (data) {
    ...
    $("#gridPDKM").dxDataGrid({
        ...
        onEditorPrepared: function (e) {
            ...
            if (e.dataField == "nilai_disbursement_pjt") {
                $(e.editorElement).dxNumberBox("instance").on("valueChanged", function (args) {
                    ...
                    var id_pel_peb = e.row.data.id_pel_peb_pib;
        ...
        onRowUpdating: function (e) {
            if (viewOnly) {
                ...
                var oldPEB = e.oldData.id_pel_peb_pib;
                //svar oldInvoice = e.oldData.nilai_invoice;



                var newPEB = e.newData.id_pel_peb_pib;
                ...
                if (newValDis <= 0) {
                    e.isValid = false;
                    e.errorText = "Nilai Barang harus lebih lebih dari 0";
        ...
        summary: {
            ...
            totalItems: [{
                column: "id_pel_peb_pib",
                showInColumn: "id_pel_peb_pib",
                    ...
                    if (TDis < 1 || TDis == null) {
                        ...
                        for (i = 0; i < DataPEB.length; i++) {
                            ...
                            for (y = 0; y < $("#gridSelectedPEB").dxDataGrid("instance").getDataSource().items()[i].total_split; y++) {
                                ...
                                obj["id_pel_peb_pib"] = $("#gridSelectedPEB").dxDataGrid("instance").getDataSource().items()[i].ID_PEL_PEB;
                                obj["tanggal_invoice"] = $("#gridSelectedPEB").dxDataGrid("instance").getDataSource().items()[i].TANGGAL_INVOICE;
                                obj["valuta_fob_cif"] = $("#gridSelectedPEB").dxDataGrid("instance").getDataSource().items()[i].VALUTA_FOB;
    ...
    $("#btnBack").dxButton({
        ...
        onClick: function () {
            ...
            window.location = routeApp + 'Pelapor/PEBPJT';
        }
    }).dxButton("instance");
        ...
        onClick: function () {
            if (viewOnly) {
                ...
                confRes.done(function (dialogResult) {
                    if (dialogResult) {
                        return $.ajax({
                            url: URL + "CancelTransctionPJT?trx=10&id_pel_peb_pib=" + GetSelectedPEB(), //+ NPWP,
                            type: "POST",
                            dataType: "json",
                            success: function (data) {
                                removeLocalStorage("dokupenDataPJTE");
                                        ...
                                        if (!isNpwpExist) {
                                            ...
                                            confRes.done(function (dialogResult) {
                                                if (dialogResult) {
                                                    ...
                                                    UploadDP();
                                                    return $.ajax({
                                                        url: URL + "SubmitPJT?trx=10&peb_pib=" + GetSelectedPEB(),
                                                        type: "POST",
                                                        // dataType: "json",
                                            ...
                                            var confMes = "PJT yang diisi akan disimpan dan akan diproses. selama proses approval PJT tidak dapat diubah. apakah anda yakin?";
                                            var confRes = DevExpress.ui.dialog.confirm(confMes, "Konfirmasi");
                                            confRes.done(function (dialogResult) {
                                                if (dialogResult) {
                                                    $(".loading-overlay").show();
    ...
    function loadData() {
        ...
        for (i = 0; i < data.length; i++) {
            if (arrData.length === 0) {
                arrData.push({
                    NomorPEB: data[i]["id_pel_peb_pib"],
                    value: data[i]["nilai_disbursment_usd"]
                });
            }
            else {
                for (y = 0; y < arrData.length; y++) {
                    if (arrData[y]["NomorPEB"] == data[i]["id_pel_peb_pib"]) {
                        arrData[y]["value"] = arrData[y]["value"] + data[i]["nilai_disbursment_usd"];
                        break;
                    }
                    if (arrData[y]["NomorPEB"] != data[i]["id_pel_peb_pib"] && (arrData.length - 1 == y)) {
                        arrData.push({
                            NomorPEB: data[i]["id_pel_peb_pib"],
                            value: data[i]["nilai_disbursment_usd"]
                        });
